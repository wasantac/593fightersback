...
exports.find3 = (req, res) => {
    Torneo.find({}, "titulo premio fecha")
        .sort("-fecha")
        .limit(3)
        .exec((err, docs) => {
            res.send(docs);
...
exports.updateParticipante = (req, res) => {
    ...
    Torneo.findOne(
        { "participantes.nick": participante.nick, _id: req.params.id },
        (err, docs) => {
            if (docs) {
                res.send(false);
            } else {
        ...
        (err, docs) => {
            if (!docs) {
                ...
                Torneo.findByIdAndUpdate(
                    req.params.id,
                    {
                        participantes: docs.participantes.filter(
                            (data) =>
                                data.nick !== participante.nick &&
...
exports.findParticipante = (req, res) => {
    User.findById(req.user._id, (err, docsUser) => {
        Torneo.findOne(
            { "participantes.nick": docsUser.nick, _id: req.params.id },
            (err, docs) => {
                if (docs) {
                    res.send(false);
                } else {
